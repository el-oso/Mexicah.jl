...
The `mexicah` command-line tool is a compiled Julia app (`juliac --output-exe`) for building MEX files without opening a Julia REPL.
...
### `mexicah compile` {#mexicah-compile}
...
### `mexicah help` {#mexicah-help}

Print usage information.

## Functions must be registered {#Functions-must-be-registered}

Only functions annotated with `@mexfunction` (or registered via `Mexicah._register_mex_export`) carry type signature metadata. The CLI reads this metadata to know what concrete types to use for argument marshaling.

If you need to compile an existing function without `@mexfunction`, use `build_mex` from the Julia API and pass `input_types` and `output_types` explicitly.
