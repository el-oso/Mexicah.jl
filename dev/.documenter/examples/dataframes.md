...
The `MexicahDataFramesExt` extension (loaded automatically when `DataFrames` is in your environment) bridges Julia `DataFrame`s to MATLAB in two ways:
...
The extension activates automatically when `DataFrames` is loaded in the same Julia session. No extra registration step needed.
...
### Example: load a CSV, process in Julia, read columns in MATLAB {#Example:-load-a-CSV,-process-in-Julia,-read-columns-in-MATLAB}
...
## Value-copy (struct) API {#Value-copy-struct-API}
...
Each column becomes a field in the MATLAB struct. Supported column element types: `Float64`, `Int32`, `Int64`, `Bool`.

### Example: compute summary statistics in Julia, return as struct {#Example:-compute-summary-statistics-in-Julia,-return-as-struct}
...
> 
> **Note:** `df_to_struct` returns an `MxArray` (pointer), not a `UInt64` handle. Use this only inside a MEX wrapper where the `MxArray` is set into `plhs` directly, not returned via the normal marshaling path. For the common case, wrap the result in a handle or use the `df_get_col_f64` approach instead.
> 
...
## Column type support {#Column-type-support}
...
Columns of other types are skipped (with a warning) during `df_to_struct`. Use `df_get_col_f64` for explicit `Float64` access; convert other types in Julia before calling `df_to_struct`.
