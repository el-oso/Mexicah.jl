...
function generate_mex_source(
    ...
    using Mexicah
    import FixedSizeArrays  # spike: FSA marshaler types referenced in the generated body
    import $mod_name
...
function _render_type(T::Type)::String
    ...
    # SPIKE: FixedSizeArray (detected by name so Mexicah needn't depend on the package).
    if nameof(T) === :FixedSizeArray
        return "FixedSizeArrays.FixedSizeArray{$(_render_type(eltype(T))), $(ndims(T))}"
    end
