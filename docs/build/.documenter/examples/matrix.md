...
## Julia source (`examples/matrix_scale.jl`) {#Julia-source-examples/matrix_scale.jl}
...
For an in-place variant like `scale!(A, s)` that writes into `A` directly, Mexicah can wrap the output slot around `A`'s existing buffer — making output also zero-copy. See the Reference for details.
